...
function Products() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      url = `url?search=${filter.search}&category=${
        filter.category
...
function handleFilter(e) {
  e.preventDefault()
  let url = `/products?search=${filter.search}&company=${filter.company}&category=${filter.category}&order=${filter.order}&price=${filter.price}&shipping=${filter.shipping & 'on'}`;
  ...
  return (
    <div>
      <form className="grid grid-cols-4 gap-3 mt-10 bg-blue-200 p-5 rounded-lg">
        ...
        <div className="flex flex-col gap-2 text-center">
          <button onClick={handleFilter} className="bg-blue-500 w-full rounded-lg cursor-pointer text-white">
            SEARCH
          </button>
        </div>

        <div className="flex flex-col gap-2 text-center">
          <button type='reset' className=" bg-purple-600 w-full rounded-lg cursor-pointer text-white">
            RESET
          </button>
        </div>
      </form>
